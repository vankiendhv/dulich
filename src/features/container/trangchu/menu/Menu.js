...
function ListMenu(props) {
  ...
  // const phanquyen = () => {
  //   if (taikhoan) {
  //     var check = taikhoan.find(x => x.email === localStorage.getItem("user"));
  //     if (check) {
  //       if (check.Roles[0] === undefined) {
  //         return false;
  //       } else {
  //         return true;
  //       }
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    var idsua = user[0].id
    if (name.trim() === "" || diachi.trim() === "" || gioitinh.trim() === "" || ngaysinh.trim() === "" || sdt.trim() === "") {
